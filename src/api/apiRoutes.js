...
import fs from 'fs';
// crypto import removed (previously used for filename entropy)
...
try { ({ default: multer } = await import('multer')); }
  ...
  // Provide inert stub when multer unavailable (tests or minimal runtime environment)
  multer = function multerStub(){ return { single: ()=> (req,res,next)=>next(), array: ()=> (req,res,next)=>next(), fields: ()=> (req,res,next)=>next() }; };
  multer.diskStorage = () => ({ /* stub storage */ });
  const _multerStubApplied = true; // non-empty marker for lint
}
...
// Security helpers handled in shared routers; local ensureAuth no longer required.

// Multer storage for avatars
const avatarsDir = path.resolve(__dirname, '..', 'public', 'images', 'avatars');
try { fs.mkdirSync(avatarsDir, { recursive: true }); } catch { /* ignore mkdir error */ const _ignoreMkdir = true; }
...
// Currently unused avatar upload middleware (prefixed underscore to satisfy lint)
const _upload = multer({ storage, limits: { fileSize: Number(process.env.MAX_FILE_SIZE_BYTES || 2*1024*1024) } });
